...
"""
rename tiff files
TARGET FILE: results.txt file; inside a folder that has the "logfiles" folder as a neighbor (e.g. panorama or results folder)
CUSTOM TEXT: no effect
"""
#=======================================================
#=======================================================
#=======================================================
SCRIPT = "duplicate_finder"
import os
import sys
...
append_log(SCRIPT, TARGET_PATH, CUSTOM_TEXT, Logfile="logs/log_global_scriptSelector.txt")
import os
from os import listdir
from os.path import isfile, join
target_folder = os.path.dirname(os.path.abspath(TARGET_PATH))
target_folder = target_folder.replace("\\","/")
logfolder = target_folder + "/../logfiles"
logfile = [f for f in listdir(logfolder) if isfile(join(logfolder, f)) and f.endswith("positions_log.txt")][0]
append_log(target_folder, Logfile="logs/log_global_scriptSelector.txt")
...
def mode3D(xs,ys,zs, minMode):
    ...
    if len(count):
        ...
        if count [0] > minMode: 
            ...
            append_log(f'At least {count [0]} duplications between tiles Y{xmin}:{xmax}, X{ymin}:{ymax}')
...
if __name__ == '__main__':
    ...
    pathname, extension = os.path.splitext(TARGET_PATH)

    if os.path.exists(TARGET_PATH) == False:
        append_log("Input file does not exist!"); exit()

    # Read input
    df = pd.read_csv(TARGET_PATH, sep = '\t', header = None)  # '../../../13_rois_for_demo/panoramas/RiceRoot_Transcripts.txt', sep = '\t', header = None)
    df = df.drop(df.columns[4:], axis = 1)
    df.columns = ['x','y','z','gene'] #,'qual']
    append_log('read input xyz dataframe')

    # Prepare parameters 
    #logfile = logfile
    w = windowsize
    Ytilesize = Xtilesize if Ytilesize == None else Ytilesize 
    ...
    for yjump in range(0, panoYmax, Ytilesize):
        ...
        for xjump in range(Xtilesize, panoXmax, Xtilesize):
            ...
            if df1.empty:
                append_log(f'Found no transcripts within Y{xmin}:{xmax}, X{ymin}:{ymax}')
                continue     

            if logfile: # Plot gridlines if asked
            ...
            if logfile: # Annotate plot with XYZ shift between tiles
        ...
        for yjump in range(Ytilesize, panoYmax, Ytilesize):
            ...
            if max(shift)> 2:
                ...
                if dups > 9: 
                    totalDups += dups
                    tileOvlaps +=1 
                    #append_log(xmin,xmax, ymin,ymax, pNUM, len(duplicated))            
    ...
    if logfile: 
        ...
        if os.path.exists(logfile):
            append_log(logfile)
            sample_name = TARGET_PATH.split('_')[-2]
            append_log(sample_name)
            pos_log = pd.read_csv(logfile, header = None)
            pos_log.columns = ['time', 'name', 'x','y','z', 'x2','y2','z2']
            pos_log = pos_log[[sample_name in r for r in pos_log.name]]
            pos_log = pos_log[['X_R1' in r for r in pos_log.name]]
            append_log(str(pos_log.columns))
    ...
    append_log('TilePairs, Tile Overlaps, Total duplicated transcripts')
    append_log(str(tilePairs), str(tileOvlaps), str(totalDups))
