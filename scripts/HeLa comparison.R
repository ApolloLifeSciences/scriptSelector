#script explanation: 
"
create a plot comparing counts from a hotrun to previous HeLa validation runs (stor2 need be mapped to N:)
TARGET FOLDER: contains the summary_transcripts.txt file directly or in a subfolder
CUSTOM TEXT: no effect
"
#=======================================================
#=======================================================
#=======================================================
SCRIPT <- "HelaComparison"
...
do({
  CURRENT_RUN <- gsub(".*[/\\]","",TARGET_FOLDER)
  ...
  gene_list <- lapply(names(gene_files), function(x) {
    tab <- read.table(gene_files[[x]], sep="\t", header=T) %>% 
      as.data.table() %>% 
      mutate(hotrun=x) %>% 
      subset(!fp & !symbol=="" & (hotrun==CURRENT_RUN | !grepl("_.*_",hotrun)) ) %>% 
      group_by(hotrun,symbol) %>% 
