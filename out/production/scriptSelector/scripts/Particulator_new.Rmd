...
do({
  append_log("get min and max values")
...
```{r}
do(supertable(details[,c(4,2,5)] %>% `colnames<-`(gsub("\\."," ",colnames(.)))))
```

## Conditions

```{r}
do(supertable(general[,c(1,2,5:12)] %>% `colnames<-`(gsub("\\/","\\/ ",colnames(.)))))
```

## Settings

```{r results="asis"}
do(supertable(general[,13:19] %>% `colnames<-`(gsub("\\/","\\/ ",colnames(.)))))
```
...
do({
  ...
  ggplot(graph_data1, aes(size_bin, value, color=factor(variable))) +
    ...
    facet_wrap(~sample) +
    scale_y_continuous(label=comma, limits=c(0,max(graph_data1$value)*1.1), expand=c(0,0), labels=function(x) format(x, scientific=T)) +
    scale_x_continuous(expand=c(0,0), breaks=seq(0,600,by=200), limits=c(0,1000)) +
...
do({
  mycolor <- "blue"
  rangetext <- paste0(MINVAL,"-",MAXVAL,"nm")
  fillvector <- c(mycolor,"gray")
  names(fillvector) <- c(rangetext,"other")
  toplot <- graph_data1 %>% mutate(size=ifelse(size_bin<=MAXVAL & size_bin>=MINVAL, rangetext, "other")) %>% 
    group_by(variable, sample, size) %>% summarise(area=sum(value))
  toplot2 <- merge(subset(toplot, size==rangetext), subset(toplot, size=="other"), by=c("variable", "sample")) %>% dplyr::rename(area="area.x")
  
  ggplot(toplot, aes(variable, area/1000, fill=size)) +
    facet_wrap(~sample) +
    geom_bar(stat="identity", position="dodge") +
    geom_label(aes(label=paste0(format(area, scientific=T),"\n",round(area/(area+area.y)*100),"%") ), data=toplot2, fill="white", color=mycolor, alpha=.8) +
...
do({
  ...
  ggplot(graph_data2, aes(percentile, value, color=variable)) +
    ...
    scale_y_continuous(expand=c(0,0), limits=c(0,600), breaks=seq(0,1000,by=100)) +
...
```{r}
do(supertable(results %>% dplyr::relocate(sample) %>% `colnames<-`(gsub("\\(.*","",colnames(.)))))
```
...
```{r}
do(graph1d(size_data, "Valid Tracks"))
```
